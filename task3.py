...
class Connector:
    ...
    def get_products(self, ids: Optional[List[int]] = None) -> List[dict]:
        return self.request("GET", "products", {"ids": ids, "pagination": {"page_size": 40}})["result"]

    def get_all_products_summary(self) -> List[dict]:
        return self.request("GET", "products", {"detailed": False, "pagination": {"page_size": 40}})["result"]

    def get_new_products(self, newer_than: Optional[datetime] = None) -> List[dict]:
        if newer_than is None:
            newer_than = datetime.now() - timedelta(days=5)
        return self.request(
            "GET", "products", {"created_at": {"start": newer_than.isoformat()}, "pagination": {"page_size": 40}}
        )["result"]
